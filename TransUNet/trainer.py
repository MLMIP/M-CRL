...
def eval(model, val_loader, device, classes):
    ...
    all_hd95 = []
    model.eval()
    #figure_save_path = '../TransUNet/model/image_f5/'
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            # image.save(figure_save_path + "img%d" % i + '.png', 'PNG')
            #
            # outputs = torch.argmax(torch.softmax(output[5], dim=0), dim=0, keepdim=False).unsqueeze(0)
            ...
            # outputs.save(figure_save_path + "output%d" % i + '.png', 'PNG')
            #
            # labs = label[5]
            ...
            # labs = labs.convert('L')
            # labs.save(figure_save_path + "labs%d" % i + '.png', 'PNG')
...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:  # iterator(0,150)
        ...
        for i_batch, sampled_batch in enumerate(trainloader):  # dict ’image‘[1,12,3,224,224] 'label'[1,12,224,224]
            image_batch, label_batch = sampled_batch['image'].squeeze(0), sampled_batch['label'].squeeze(0)  # 压缩第一个维度
